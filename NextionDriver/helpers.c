...
void updateDB(int age) {
	...
	sprintf(cmd, "curl -s -L -o /tmp/groups -A 'WPSD NextionDriver v.%s' %s && mv /tmp/groups %s", NextionDriver_VERSION, groupsFileSrc, fname);
        ok=system(cmd);
        if (ok!=0) {
	...
	sprintf(cmd, "curl -s -L -A 'WPSD NextionDriver v.%s' %s -o /tmp/users >/dev/null 2>&1 && touch /tmp/users && mv /tmp/users %s", NextionDriver_VERSION, usersFileSrc, fname);
        ok=system(cmd);
        if (ok!=0) {
